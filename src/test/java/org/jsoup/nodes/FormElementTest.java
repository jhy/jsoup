...
public class FormElementTest {
    ...
    public void hasAssociatedControls() {
        ...
        String html = "<form id=1><button id=1><fieldset id=2 /><input id=3><keygen id=4><object id=5><output id=6>" + "<select id=7><option></select><textarea id=8><p id=9>";
    ...
    @Test
    public void formDataUsesFirstAttribute() {
    ...
    @Test
    public void formSubmissionCarriesCookiesFromSession() throws IOException {
        // this is a dirty hack to initialize the EchoServlet(!)
        String echoUrl = EchoServlet.Url;
        Document cookieDoc = Jsoup.connect(CookieServlet.Url).data(CookieServlet.SetCookiesParam, "1").get();
        Document formDoc = // carries cookies from above set
        cookieDoc.connection().newRequest().url(FileServlet.urlTo("/htmltests/upload-form.html")).get();
