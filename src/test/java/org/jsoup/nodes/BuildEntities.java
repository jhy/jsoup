...
class BuildEntities {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Connection.Response res = Jsoup.connect(url).ignoreContentType(true).userAgent(UrlConnectTest.browserUa).execute();
        Gson gson = new Gson();
        Map<String, CharacterRef> input = gson.fromJson(res.body(), new TypeToken<Map<String, CharacterRef>>() {
        }.getType());
        ...
        for (Map.Entry<String, CharacterRef> entry : input.entrySet()) {
            // name is like &acute or &acute; , trim &
    ...
    private static class CharacterRef {
        ...
        public String toString() {
            return name + "=" + d(codepoints[0]) + (codepoints.length > 1 ? "," + d(codepoints[1]) : "") + ";" + d(codeIndex);
        }
    }
    ...
    private static class ByCode implements Comparator<CharacterRef> {
        ...
        public int compare(CharacterRef o1, CharacterRef o2) {
            ...
            if (c1.length == 1 && c2.length == 1) {
                // for the same code, use the shorter name
            ...
            if (c1.length == 2 && c2.length == 2)
                ...
                // pushes multi down the list so hits on singles first (don't support multi lookup by codepoint yet)
