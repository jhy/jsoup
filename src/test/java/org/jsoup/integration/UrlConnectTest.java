...
@Disabled
public class // todo: rebuild these into a local Jetty test server, so not reliant on the vagaries of the internet.
UrlConnectTest {
    ...
    public void exceptOnUnknownContentType() {
        // not text/* but image/png, should throw
    ...
    public void handlesEscapedRedirectUrls() throws IOException {
        ...
        // i.e. double escaped
        Connection.Response res = Jsoup.connect(url).proxy("localhost", 8888).execute();
        Document doc = res.parse();
        assertEquals(200, res.statusCode());
    }
    ...
    @Test
    public void canRequestIdn() throws IOException {
