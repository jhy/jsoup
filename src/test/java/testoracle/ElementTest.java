...
public class ElementTest {
	...
	public void testGetElementById() {
		...
		Element span = div2.child(0).getElementById("2"); // called from <p>
															// context should be
															// span
	...
	public void testSetText() {
		...
		Document doc = Jsoup.parse(h);
		assertEquals("Hello there now", doc.text()); // need to sort out node
														// whitespace
	...
	public void testAddNewElement() {
		...
		// manually specifying tag and attributes should now preserve case,
		// regardless of parse mode
		assertEquals(
	...
	public void testWrap() {
		...
		assertEquals("<div><div class=\"head\"><p>Hello</p></div><p>There</p></div>",
				TextUtil.stripNewlines(doc.body().html()));
	...
	public void before() {
		...
		assertEquals("<div><div>one</div><div>two</div><p>Hello</p><p>There</p></div>",
				TextUtil.stripNewlines(doc.body().html()));

		doc.select("p").last().before("<p>Three</p><!-- four -->");
		assertEquals("<div><div>one</div><div>two</div><p>Hello</p><p>Three</p><!-- four --><p>There</p></div>",
				TextUtil.stripNewlines(doc.body().html()));
	}
	...
	public void after() {
		...
		assertEquals("<div><p>Hello</p><div>one</div><div>two</div><p>There</p></div>",
				TextUtil.stripNewlines(doc.body().html()));

		doc.select("p").last().after("<p>Three</p><!-- four -->");
		assertEquals("<div><p>Hello</p><div>one</div><div>two</div><p>There</p><p>Three</p><!-- four --></div>",
				TextUtil.stripNewlines(doc.body().html()));
	}
	...
	public void testWrapWithRemainder() {
		...
		assertEquals("<div><div class=\"head\"><p>Hello</p><p>There!</p></div></div>",
				TextUtil.stripNewlines(doc.body().html()));
	}
	...
	public void testClone() {
		...
		assertEquals("<div><p>One</p><p><span>Two</span></p></div>", TextUtil.stripNewlines(doc.body().html())); // not
																													// modified

		doc.body().appendChild(clone); // adopt
		assertNotNull(clone.parent());
		assertEquals("<div><p>One</p><p><span>Two</span></p></div><p><span>Two</span><span>Three</span></p>",
				TextUtil.stripNewlines(doc.body().html()));
	}
	...
	public void moveByAppend() {
		...
		assertEquals(0, children.size()); // children is backed by
											// div1.childNodes, moved, so should
											// be 0 now
	...
	public void insertChildrenAtPosition() {
		Document doc = Jsoup
				.parse("<div id=1>Text1 <p>One</p> Text2 <p>Two</p></div><div id=2>Text3 <p>Three</p></div>");
		Element div1 = doc.select("div").get(0);
	...
	public void testHashAndEqualsAndValue() {
		...
		/*
		 * for (Element el : els) { System.out.println(el.hashCode() + " - " +
		 * el.outerHtml()); }
		 * 
		 * 0 1534787905 - <p class="one">One</p> 1 1534787905 - <p
		 * class="one">One</p> 2 1539683239 - <p class="one">Two</p> 3
		 * 1535455211 - <p class="two">One</p> 4 1534787905 - <p
		 * class="one">One</p> 5 1534787905 - <p class="one">One</p> 6
		 * 1539683239 - <p class="one">Two</p> 7 1535455211 - <p
		 * class="two">One</p>
		 */
	...
	public void testChainedRemoveAttributes() {
		...
		a.removeAttr("zero").removeAttr("one").removeAttr("two").removeAttr("three").removeAttr("four")
				.removeAttr("five");
	...
	@Test
	public void TestDataFunctionInDataNode() {
		Document doc = Jsoup.parse("<script>One</script> <style>Two</style><p>Three</p>");
		Element script = doc.select("script").first();

		assertEquals("One", script.data());
	}

	@Test
	public void TestDataFunctionInElement() {
		Document doc2 = Jsoup.parse("<div><p><a>One</a></p><b>Two</b><p>Three</p>");
		Element p2 = doc2.select("p").first();
		assertEquals("<a>One</a>", p2.data()); //return false..
	}

	@Test
	public void TestDataFunctionInComment() {
		Document doc3 = Jsoup.parse("<div><p><!--One--></p><a>Two</a><p>Three</p>");
		Element p3 = doc3.select("p").first();

		assertEquals("One", p3.data());
	}
	...
	public void TestAttributes() {
		...
		assertNotNull(p.getElementsByAttributeValueNot("data","test"));
		assertNotNull(p.getElementsByAttributeValueStarting("data", "test"));
		assertNotNull(p.getElementsByAttributeValueEnding("data", "test"));
		assertNotNull(p.getElementsByAttributeValueContaining("data", "test"));
		assertNotNull(p.getElementsByAttributeValueMatching("data", Pattern.compile("test") ));
		assertNotNull(p.getElementsByAttributeValueMatching("data", "test"));
		assertNotNull(p.getElementsByIndexLessThan(0));
		assertNotNull(p.getElementsByIndexGreaterThan(0));
		assertNotNull(p.getElementsByIndexEquals(0));
		assertNotNull(p.getElementsContainingText("test"));
		assertNotNull(p.getElementsContainingOwnText("test"));
		assertNotNull(p.getElementsMatchingText("test"));
		assertNotNull(p.getElementsMatchingOwnText(Pattern.compile("test")));
		assertNotNull(p.getElementsMatchingOwnText("test"));
	}
