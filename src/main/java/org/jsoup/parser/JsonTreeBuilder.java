 ...
 * @author Jesse Wilson
 *
 * see com.google.gson.stream.JsonReader, com.google.gson.stream.JsonReader#setLenient
 */
...
public class JsonTreeBuilder extends XmlTreeBuilder {
  ...
  protected enum VALUE_CLASS {
    ...
    VALUE_CLASS (String str, String num) {
      cssClassString = str;  cssClassNumeric = num;
    }
  ...
  NEXT_TOKEN nextToken () {
    ...
    case ARRAY:
      c = nextNonWhitespace();
      switch (c) {
      case ']': case '}': case EOF: return NEXT_TOKEN.END_ARRAY;// [value>]
      case ';': case ',': break;
    ...
    case OBJECT:
      ...
      while(true) {
        ...
        switch (c) {
        case ';': case ',': break;//comma before the next element
        case '"':  return NEXT_TOKEN.DOUBLE_QUOTED_NAME;
        case '\'': return NEXT_TOKEN.SINGLE_QUOTED_NAME;
        case '}': case ']': case EOF: return NEXT_TOKEN.END_OBJECT;// ,} is OK too
        default:
          reader.unconsume();//Don't consume the first character in an unquoted string
    ...
    case DANGLING_NAME:
      ...
      c = nextNonWhitespace();
      switch (c) {
      case EOF: return NEXT_TOKEN.NULL;
      case ':': break;//a colon before the value !skip
      case '=':// = or => !skip
        if (reader.matches('>')) {
          reader.advance();
        }
    ...
    case NONEMPTY_DOCUMENT:
      c = nextNonWhitespace();
      switch (c) {
      case EOF: return NEXT_TOKEN.EOF;
      case ',': case ';': break;//not in JSON standard, but we allow val1,val2,...
      default:
        reader.unconsume();//Don't consume the first character in an unquoted string
      }
    }//scope
    ...
    case '\'': return NEXT_TOKEN.SINGLE_QUOTED;
    case '"':  return NEXT_TOKEN.DOUBLE_QUOTED;
    case '[':  return NEXT_TOKEN.BEGIN_ARRAY;
    case '{':  return NEXT_TOKEN.BEGIN_OBJECT;
    case ':': case '=':
  ...
  @Override protected boolean process (Token token) {
    throw new AssertionError("process(Token) - must not be called, but "+token);
  }


  @Override protected void runParser () {
    while (true) {
  ...
  Element insertStartTag (String tagName) {
    ...
    insert(el);
//if (!tag.isKnownTag())/*unknown tag, remember this is self closing for output. see above*/ tag.setSelfClosing();
    stack.add(el);
  ...
  protected boolean isNumeric (CharSequence s) {
    ...
    while (i < len) {
      ...
      case 'e': case 'E':
        ...
        if (dec == 1) { dec--;}//second chance for .
        break;
      default:
